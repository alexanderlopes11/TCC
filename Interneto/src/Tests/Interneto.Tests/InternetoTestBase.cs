...
using Interneto.Migrations.SeedData;
...
namespace Interneto.Tests
{
    public abstract class InternetoTestBase : AbpIntegratedTestBase<InternetoTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected InternetoTestBase()
        {
        ...
        protected void UsingDbContext(Action<InternetoDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<InternetoDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<InternetoDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<InternetoDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<InternetoDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<InternetoDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<InternetoDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<InternetoDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<InternetoDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<InternetoDbContext, Task<T>> func)
        {
            T result;
